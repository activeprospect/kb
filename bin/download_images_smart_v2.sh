...
# Smart image downloader with change detection and deduplication
# This script will:
# 1. Find all markdown files in a given directory
# 2. Check if the file actually needs processing (has remote URLs)
# 3. Parse image links from the markdown content and screenshot URLs from frontmatter
# 4. Download images to article-specific subdirectories to avoid conflicts
# 5. Use content-based hashing for deduplication
# 6. Generate descriptive filenames based on alt text or context
# 7. Only update markdown files if changes were actually made
...
needs_processing() {
    ...
    # Check for remote images in content
    if echo "$content" | grep -qE '!\[[^]]*\]\(https?://[^)]+\)'; then
        return 0
    fi
    
    # Check for remote screenshot URL in frontmatter
    local screenshot_url=$(echo "$content" | awk '/^---$/{p++} p==1{print} p==2{exit}' | grep "^Screenshot URL:" | sed 's/^Screenshot URL: //')
...
process_markdown_file() {
    ...
    if [[ -n "$screenshot_url" ]] && [[ "$screenshot_url" =~ ^https?:// ]]; then
        ...
        if [[ "$DRY_RUN" == false ]]; then
            ...
            if [[ -n "$result" ]]; then
                ...
                local new_screenshot_ref="${file_basename}-images/$screenshot_filename"
                processed_content=$(echo "$processed_content" | sed "s|^Screenshot URL: .*|Screenshot URL: $new_screenshot_ref|")
            ...
            changes_made=true  # Would make changes if not dry-run
        fi
    fi
    ...
    # Only update the markdown file if actual changes were made
    if [[ "$changes_made" == true ]] && [[ "$DRY_RUN" == false ]]; then
        # Double-check that content actually changed
        if [[ "$processed_content" != "$original_content" ]]; then
...
while IFS= read -r -d '' md_file; do
    ...
    if needs_processing "$md_file"; then
        ...
        ((files_skipped++))
        echo "⏭️  Skipping: $md_file (no remote URLs)"
    fi
...
echo "   Files needing processing: $files_needing_processing"
echo "   Files skipped (no remote URLs): $files_skipped"
...
if [[ "$DRY_RUN" == true ]]; then
    ...
    if [[ $total_image_files -gt 0 ]] || [[ $total_symlinks -gt 0 ]]; then
